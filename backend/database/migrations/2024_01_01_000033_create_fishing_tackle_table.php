...
return new class extends Migration
{
    ...
    {
        Schema::create('fishing_tackle', function (Blueprint $table) {
            $table->id();
            $table->string('name'); // Название снасти
            $table->string('brand')->nullable(); // Бренд
            $table->string('model')->nullable(); // Модель
            $table->string('slug')->unique(); // URL slug
            $table->text('description')->nullable(); // Описание
            $table->string('type')->default('rod'); // rod, reel, line, hook, lure, bait, etc.
            $table->string('category')->nullable(); // Подкатегория
            ...
            $table->string('photo_url')->nullable(); // Фото снасти
            ...
            $table->json('compatible_with')->nullable(); // Совместимость
    ...
    {
        Schema::dropIfExists('fishing_tackle');
    }
};
