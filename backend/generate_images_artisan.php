...
/**
 * Генератор изображений рыб через Artisan
 */

echo "🎨 Генерация изображений рыб через Artisan...\n";

// Создаем директорию для изображений
$imageDir = __DIR__ . '/public/images/fish';
if (!is_dir($imageDir)) {
    mkdir($imageDir, 0755, true);
    echo "📁 Создана директория: {$imageDir}\n";
}
...
// Виды рыб для генерации
$fishTypes = [
    'Щука', 'Окунь', 'Лещ', 'Плотва', 'Карась', 'Карп', 
    'Сазан', 'Сом', 'Судак', 'Жерех', 'Голавль', 'Язь',
    'Линь', 'Красноперка', 'Уклейка', 'Ерш', 'Пескарь',
    'Бычок', 'Ротан', 'Толстолобик'
];

echo "🎣 Генерируем изображения для 20 видов рыб...\n\n";

$generated = 0;
foreach ($fishTypes as $index => $fishType) {
    $fishId = $index + 1;
    ...
    if ($image) {
        ...
        echo "❌ Ошибка создания изображения для {$fishType}\n";
    }
}

echo "\n🎉 Генерация завершена!\n";
echo "📊 Создано изображений: {$generated}/20\n";
